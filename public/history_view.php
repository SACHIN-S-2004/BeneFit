...
// Validate ?id= param
$diet_result_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
if ($diet_result_id <= 0) {
    header("Location: dashboard.php");
    exit;
}

// ─── Fetch diet result + health inputs (ownership check via user_id) ───
$stmt = $conn->prepare("
    SELECT dr.id          AS dr_id,
           ...
           dr.created_at,
           hi.age,        hi.gender,
           hi.height_cm,  hi.weight_kg,  hi.bmi,
           hi.bmr,        hi.tdee,
           hi.daily_calorie_target,
           hi.daily_water_liters,
           hi.protein_g,  hi.carbs_g,    hi.fats_g,
           hi.activity,   hi.disease
    FROM   diet_results  dr
    JOIN   health_inputs hi ON dr.health_input_id = hi.id
    WHERE  dr.id = ?
      AND  dr.user_id = ?
");
$stmt->bind_param("ii", $diet_result_id, $user_id);
$stmt->execute();
...
// ─── Fetch saved food ids grouped by meal ────────────────────────────────
$foods_stmt = $conn->prepare("
    SELECT meal_type, food_id
    FROM   diet_result_foods
    WHERE  diet_result_id = ?
    ORDER BY FIELD(meal_type, 'breakfast', 'lunch', 'dinner'), id
");
$foods_stmt->bind_param("i", $diet_result_id);
$foods_stmt->execute();
$foods_result = $foods_stmt->get_result();
$foods_stmt->close();

$saved_food_ids = ['breakfast' => [], 'lunch' => [], 'dinner' => []];
while ($f = $foods_result->fetch_assoc()) {
    $saved_food_ids[$f['meal_type']][] = (int)$f['food_id'];
}

// ─── Load food CSV & build lookup by id ──────────────────────────────────
$food_lookup = [];
if (($handle = fopen("/var/www/datasets/daily_food_nutrition_dataset.csv", "r")) !== false) {
    $headers = fgetcsv($handle);
    $headers = array_map('trim', $headers);
    while (($csv_row = fgetcsv($handle)) !== false) {
        $item = array_combine($headers, $csv_row);
        $food_lookup[(int)$item['id']] = $item;
    }
    fclose($handle);
}
...
foreach (['breakfast', 'lunch', 'dinner'] as $meal) {
    $meals[$meal] = [];
    foreach ($saved_food_ids[$meal] as $fid) {
        if (isset($food_lookup[$fid])) {
            $meals[$meal][] = $food_lookup[$fid];
        }
    }
}

// ─── Reconstruct display variables (same naming as result.php) ───────────
$response = [
    'goal'       => $row['goal'],
    'final_diet' => $row['final_diet'],
];
...
// ─── Meal UI config (identical to result.php) ────────────────────────────
$meal_config = [
    "breakfast" => [
...
<title>Diet Plan — <?= htmlspecialchars($created_at) ?></title>
...
<body>
  ...
  <div class="container py-5">
    ...
    <!-- History breadcrumb badge -->
    <div class="mb-4 fade-up">
      <span class="badge bg-secondary bg-opacity-15 border border-secondary border-opacity-20 rounded-pill px-3 py-2">
        <i class="bi bi-clock-history me-1"></i> Viewing saved plan from <?= htmlspecialchars($created_at) ?>
      </span>
    </div>
    ...
    <div class="row g-5">
        ...
        foreach (["breakfast", "lunch", "dinner"] as $meal):
            ...
            $color    = $config['color'];
            $bg_style = "background: {$color}20; color: $color;";
        ?>
        <div class="col-12 fade-up delay-<?= $delay_counter++ ?>">
            ...
            <div class="row g-4">
                <?php if (!empty($meals[$meal])): ?>
                    <?php foreach ($meals[$meal] as $food): ?>
                        ...
                        <div class="glass-card p-4">
                            ...
                            <div class="d-flex gap-2 mt-auto">
                                <div class="nutrition-pill">
                                    ...
                                    <?= htmlspecialchars($food["Calories (kcal)"]) ?> kcal
                                </div>
                                <div class="nutrition-pill">
                                    <i class="bi bi-egg-fried text-warning"></i>
                                    <?= htmlspecialchars($food["Protein (g)"]) ?>g Protein
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <?php else: ?>
                    <div class="col-12">
                        <p class="text-secondary opacity-50">No foods saved for this meal.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
function toggleTheme() {
  ...
  btn.className = newTheme === 'dark' ? 'bi bi-moon-stars-fill' : 'bi bi-sun-fill';
...
(function () {
  ...
  const btn = document.querySelector('button[onclick="toggleTheme()"] i');
  if (savedTheme === 'light') btn.className = 'bi bi-sun-fill';
})();
</script>
